...
import sys
sys.path.insert(0, '../')
from model.model_longcyclip import build_model
...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", download_root: str = None, dropout: bool = False, add_batch_norm: bool = False):
    ...
    model = build_model(state_dict or model.state_dict(), load_from_clip = False, dropout=dropout, add_batch_norm=add_batch_norm).to(device)
...
def load_from_clip(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None, dropout=False, add_batch_norm=False):
    ...
    model = build_model(state_dict or model.state_dict(), load_from_clip = True, dropout=dropout, add_batch_norm=add_batch_norm).to(device)
